...
public class RhService {
    ...
    public Map<Long, Integer> nbjCongeUtilise(List<Employe> employes) {
        ...
        for (Object[] ligne : resultats) {
            Number idEmployeNumber = (Number) ligne[0];
            Number sommeNumber = (Number) ligne[1];

            Long idEmploye = idEmployeNumber.longValue(); // conversion sûre
            Integer somme = sommeNumber.intValue();       // ou .longValue() si tu veux garder Long

            map.put(idEmploye, somme);
        }
